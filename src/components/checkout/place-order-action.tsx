...
export const PlaceOrderAction: React.FC<{ className?: string }> = (props) => {
  ...
  const handlePlaceOrder = () => {
    ...
    if (payment_gateway === 'STRIPE') {
      ...
      input.token = token;
        ...
        delete input.billing_address.address?.__typename;
        // delete input.shipping_address.__typename;
        delete input.shipping_address.address?.__typename;    
    //@ts-ignore
    createOrder(input);
  };
  ...
  return (
    <>
      <Button
        loading={isLoading}
        className={classNames('mt-5 w-full text-red-450', props.className)}
