...
export const GuestAddressGrid: React.FC<AddressesProps> = ({
  ...
  function onDuplicate(address: any) {
    type === 'billing' ? (atom = shippingAddressAtom ) : (atom = billingAddressAtom);
    type === 'billing' ? (type = 'shipping') : (type = 'billing');
    address?.type === 'billing' ? (address.type = 'shipping') : (address.type = 'billing');
    openModal('ADD_OR_UPDATE_GUEST_ADDRESS', { type, atom, address });
  }
              ...
              <RadioGroup.Option value={address} key={address?.id}>
                {({ checked }) => (
                  <AddressCard
                    ...
                    onDuplicate={() => onDuplicate(address)}
                  />
                )}
              </RadioGroup.Option>
            ))}
