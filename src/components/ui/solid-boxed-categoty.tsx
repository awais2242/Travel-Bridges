...
const CategoryItem: React.FC<CategoryItemProps> = ({ item, is_link}) => {
  const router = useRouter();

  const { pathname, query } = router;
  const selectedQueries = query.category;

  const onCategoryClickLink = (slug: string) => {
    if (selectedQueries === slug) {
      const { category, ...rest } = query;
      router.push(
        {
          pathname: `/${item?.type?.slug}${Routes.search}`,
          query: { ...rest },
        },
        undefined,
        {
  ...
  return (
    <div
      ...
      role="button"
      onClick={is_link?(() => onCategoryClickLink(item?.slug!)) : (() => onCategoryClick(item?.slug!))}
    >
...
function SolidBoxedCategoryMenu({ items, is_link }: any) {
    ...
    <div className="relative">
      ...
      >
        {items?.map((category: any, idx: number) => (
          <SwiperSlide key={idx}>
            <CategoryItem item={category} is_link={is_link}/>
          </SwiperSlide>
        ))}
      </Swiper>
      <div
