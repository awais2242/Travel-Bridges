...
const variantClasses = {
  ...
  neon: 'w-full h-7 md:h-9 bg-red-450 text-light rounded-none',
  argon:
  ...
  single:
    'order-5 sm:order-4 w-9 sm:w-24 h-24 sm:h-10 bg-red-450 text-light rounded-full flex-col-reverse sm:flex-row absolute sm:relative bottom-0 sm:bottom-auto ltr:right-0 rtl:left-0 ltr:sm:right-auto ltr:sm:left-auto',
  details:
    'order-5 sm:order-4 w-full sm:w-24 h-10 bg-red-450 text-light rounded-full',
  pillVertical:
    'flex-col-reverse items-center w-8 h-24 bg-gray-100 text-heading rounded-full',
  big: 'w-full h-14 rounded text-light bg-red-450 inline-flex justify-between',
  text: 'w-7 h-18 sm:w-20 sm:h-7 md:h-9 md:w-24 bg-red-450 flex-col-reverse sm:flex-row text-light rounded',
  bordered:
  ...
  return (
    ...
    >
      <button
        onClick={onDecrement}
        className={cn(
          'cursor-pointer p-2 transition-colors duration-200 hover:bg-red-450 focus:outline-none',
          {
            'px-3 py-3 sm:px-2': variant === 'single',
            'px-5': variant === 'big',
            'border border-red-450 px-5 hover:border-red-450 hover:!bg-transparent ltr:rounded-l rtl:rounded-r':
              variant === 'bordered',
      ...
      <button
        ...
        className={cn(
          'cursor-pointer p-2 transition-colors duration-200 hover:bg-red-450 focus:outline-none',
          {
            'px-3 py-3 sm:px-2': variant === 'single',
            'px-5': variant === 'big',
            'border border-red-450 px-5 hover:border-red-450 hover:!bg-transparent hover:!text-accent ltr:rounded-r rtl:rounded-l':
              variant === 'bordered',
            'hover:!bg-red-450': variant === 'pillVertical',
          }
        )}
