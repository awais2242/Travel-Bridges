  ...
  return (
    <section className="relative flex h-full flex-col">
      ...
      <footer className="fixed bottom-0 z-10 w-full max-w-md bg-light py-5 px-6">
        <div className='grid grid-cols-2 py-5 border-t-2'>
          <div>
            <span className="text-sm font-semibold text-heading">{t('text-total')}</span>
          </div>
          <div>

            <span className="flex h-full shrink-0 items-center rounded-full bg-light px-5 text-accent float-right">
              {totalPrice}
            </span>
          </div>
        ...
        <button
          className="flex h-12 w-full justify-between border-2 uppercase p-1 text-red-450 hover:text-light text-sm font-bold shadow-700 transition-colors hover:bg-red-450 focus:outline-none md:h-14"
          onClick={handleCheckout}
        >
          <span className="flex h-full flex-1 items-center justify-center px-5  ">
            {t('text-checkout')}
          </span>
        </button>
      </footer>
